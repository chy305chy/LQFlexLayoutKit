...
/* when view's content (eg: UILabel's text) changed, ignore cached layout and measure new layout according to new content
 * default value is NO
 * CAUTION: 如果开启了ignoreCache（设置shouldIgnoreCachedLayout=YES），UIView会强引用YGLayoutConfigurationBlock，
 * 此时，在设置YGLayoutConfigurationBlock时，要避免在其中直接使用self或者self的实例变量（会造成retain cycle）
 */
@property (nonatomic, readwrite, assign) BOOL shouldIgnoreCachedLayout;
