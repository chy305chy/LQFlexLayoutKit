...
@synthesize shouldIgnoreCachedLayout = _shouldIgnoreCachedLayout;
...
- (void)applyLayout
{
    [LQAsyncFlexLayoutTransaction addAsyncCalculateTransaction:^{
...
-(void)applyLayoutASyncPreservingOrigin:(BOOL)preserveOrigin {
    [LQAsyncFlexLayoutTransaction addAsyncCalculateTransaction:^{
        [self calculateLayoutWithSize:self.view.bounds.size];
    } complete:^{
        YGApplyLayoutToViewHierarchy(self.view, preserveOrigin);
...
-(void)applyLayoutASyncPreservingOrigin:(BOOL)preserveOrigin dimensionFlexibility:(YGDimensionFlexibility)dimensionFlexibility {
    CGSize size = self.view.bounds.size;
    if (dimensionFlexibility & YGDimensionFlexibilityFlexibleWidth) {
        size.width = YGUndefined;
    }
...
- (CGSize)calculateLayoutWithSize:(CGSize)size
{
    ...
    [self _checkIgnoreCachedLayout:self.view];
...
- (void)_checkIgnoreCachedLayout:(UIView *)view {
    if (view.yoga.isEnabled && view.yoga.shouldIgnoreCachedLayout) {
        [view resetYogaIfNeeded];
    }
    for (UIView *subview in view.subviews) {
        [self _checkIgnoreCachedLayout:subview];
    }
}
